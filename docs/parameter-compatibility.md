...
`config.example.yaml` intentionally does not define per-model `defaults`. OpenAI-compatible providers do not fully agree on sampling and reasoning parameters.
